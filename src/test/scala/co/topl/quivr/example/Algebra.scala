...
object SimpleExpr {
  ...
  case class Sum(left: SimpleExpr.Num, right: SimpleExpr.Num) extends SimpleExpr[Int]{ val value: Int = left.value + right.value }
  case class Or(left: SimpleExpr.Bool, right: SimpleExpr.Bool) extends SimpleExpr[Boolean]{ val value: Boolean = left.value || right.value }
  case class And(left: SimpleExpr.Bool, right: SimpleExpr.Bool) extends SimpleExpr[Boolean]{ val value: Boolean = left.value && right.value }
  ...
  implicit val simpleExprAlg: Algebra[SimpleExpr] = new Algebra[SimpleExpr] {
    ...
    override def or(left: SimpleExpr[Boolean], right: SimpleExpr[Boolean]): SimpleExpr[Boolean] = SimpleExpr.Bool(left.value || right.value)
    override def and(left: SimpleExpr[Boolean], right: SimpleExpr[Boolean]): SimpleExpr[Boolean] = SimpleExpr.Bool(left.value && right.value)
    override def not(boolean: SimpleExpr[Boolean]): SimpleExpr[Boolean] = SimpleExpr.Bool(!boolean.value)
    override def sum(left: SimpleExpr[Int], right: SimpleExpr[Int]): SimpleExpr[Int] = SimpleExpr.Num(left.value + right.value)
  }
}
